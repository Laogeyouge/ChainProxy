...
class MainWindow(QMainWindow):
    ...
    def _final_cleanup(self):
        ...
        try:
            ...
            pass
        # Orphan sweep: belt-and-suspenders kill of any mihomo we lost
        # track of. Common path on Windows: TUN-mode UAC prompt cancelled,
        # the runner cleared uac_pid, mihomo kept running and surprised
        # the user in Task Manager. macOS uses panic_recover (heavier —
        # also wipes utun routes that mihomo's clean shutdown would have
        # removed). Pass a no-op log so we don't try to write to a Qt UI
        # that's mid-teardown.
    ...
    def closeEvent(self, e):
        ...
        # Explicit quit (tray menu / Ctrl+Q). Run cleanup synchronously so
        # mihomo is dead and proxy is restored before we accept close. Then
        # call QApplication.quit() explicitly: the typical path here is
        # X-button-to-tray (hides MainWindow) THEN tray-quit, so by the
        # time e.accept() runs the window is already hidden. Qt's
        # quitOnLastWindowClosed only fires on a visible→closed transition,
        # not a hidden→closed one, so without the explicit quit() the event
        # loop runs forever and ChainProxy.exe lingers in Task Manager.
        try:
            self._final_cleanup()
        except Exception:
            pass
